...
public class SendCommands {
    ...
    public int SendAdbCommands(Context context, final byte[] fileBase64, final String ip, int port, String localip, int bitrate, int size) {
    ...
    private void adbWrite(String ip, int port, byte[] fileBase64, String command) throws IOException {
        ...
        try {
            ...
            status = 2;
            throw new ConnectException("Device at " + ip + ":" + Integer.toString(port) + " has no adb enabled or connection is refused");
        } catch (NoRouteToHostException e) {
            status = 2;
            throw new NoRouteToHostException("Couldn't find adb device at " + ip + ":" + Integer.toString(port));
        } catch (IOException e) {
            e.printStackTrace();
            status = 2;
        }
